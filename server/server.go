...
func (s *Server) StartServer() error {
	...
	go s.listen(listenMsg)
	err := s.process(messagesChan)
	return err
}

func (s *Server) listen(messagesChan chan byte) {

	receivedMessages := make(chan *amqp.Delivery)
	errChan := s.receive(receivedMessages)
	for {
		select {
		case <-s.ctx.Done():
			return
...
func (s *Server) receive(messages chan *amqp.Delivery) chan error {
...
func (s *Server) process(messagesChan chan *amqp.Delivery) error {
	for {
		select {
		case <-s.ctx.Done():
			return nil
...
func (s *Server) Item(item *types.Item) (logMessage string) {
	switch item.Method {
	case types.AddI:
		s.data.Lock()
		defer s.data.Unlock()
